...
def bad_request_error(value: Any) -> ApiError:
    return ApiError(status_code=400, detail=str(value))


def not_found_error(value: Any) -> ApiError:
    return ApiError(status_code=404, detail=str(value))


def upstream_error(value: Any) -> ApiError:
    return ApiError(status_code=502, detail=str(value))


def service_unavailable_error(value: Any) -> ApiError:
    return ApiError(status_code=503, detail=str(value))
