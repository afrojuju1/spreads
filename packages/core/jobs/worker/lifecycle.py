...
async def _enqueue_startup_discovery_recovery(ctx: dict[str, Any]) -> None:
    job_store = ctx["job_store"]
    definition = await asyncio.to_thread(get_declared_job_row, DISCOVERY_RECOVERY_JOB_KEY)
    ...
    if latest_run is not None and str(latest_run.get("status") or "") in {
        ...
        return
    spec = get_job_spec(DISCOVERY_RECOVERY_JOB_TYPE)
    if spec is None:
        return
    redis = await create_pool(build_redis_settings(ctx["redis_url"]))
    try:
        scheduled_for = datetime.now(UTC)
        job_run_id = f"discovery_recovery:start:{uuid4().hex}"
...
async def runtime_startup(ctx: dict[str, Any]) -> None:
    ...
    await startup(ctx)
    await _enqueue_startup_discovery_recovery(ctx)
