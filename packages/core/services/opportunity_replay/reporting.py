        ...
        rows.append(
            {
                ...
                "allocation_reason": None if allocation is None else allocation.allocation_reason,
                ...
                "actual_position_status_counts": outcome.get("actual_position_status_counts"),
                ...
                "close_execution_attempt_ids": outcome.get("close_execution_attempt_ids"),
                ...
                "entry_limit_retention_total": outcome.get("entry_limit_retention_total"),
                "entry_limit_retention_count": outcome.get("entry_limit_retention_count"),
                ...
                "request_to_force_close_total": outcome.get("request_to_force_close_total"),
                "request_to_force_close_count": outcome.get("request_to_force_close_count"),
                ...
                "is_promotable_baseline": opportunity.baseline_selection_state
                == "promotable",
                "is_monitor_baseline": opportunity.baseline_selection_state
                == "monitor",
        ...
        result.append(
            {
                ...
                "allocator_promoted_monitor_count": sum(
                    1 for row in group_rows if row.get("is_allocator_promoted_monitor")
                ),
...
def _build_scorecard(
    *,
    ...
    promotable_baseline = [
        item for item in opportunities if item.baseline_selection_state == "promotable"
    ]
    allocator_promoted_monitor = [
        item
        for item in allocator_selected
        if item.baseline_selection_state == "monitor"
    ]
...
def _build_comparison(
    *,
    ...
    monitor_baseline = [
        item for item in opportunities if item.baseline_selection_state == "monitor"
    ]
    allocated = [
        item
        for item in opportunities
...
def _build_summary(
    *,
    ...
    promotable_baseline_symbols = sorted(
        {
            item.symbol
            for item in opportunities
            if item.baseline_selection_state == "promotable"
        }
    )
    ...
    summary = {
        ...
        "promotable_count": sum(1 for item in opportunities if item.state == "promotable"),
