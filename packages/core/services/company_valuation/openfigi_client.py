...
class OpenFigiClient:
    def __init__(
        ...
        *,
        ...
        self.http = VendorHttpClient(timeout_seconds=self.timeout_seconds, user_agent="spreads-openfigi/1.0")
    ...
    def map_cusips(
        ...
        try:
            decoded = self.http.request_json("POST", self.base_url, "/v3/mapping", body=payload, headers=headers)
        except VendorHttpError as exc:
            raise OpenFigiRequestError(
                f"OpenFIGI mapping failed: {exc.status_code or 'transport'} {exc.reason or ''}".rstrip(),
        ...
        for cusip, item in zip(normalized, decoded, strict=False):
            ...
            if not isinstance(rows, list):
                continue
            results[cusip] = [OpenFigiMapping.from_payload(row) for row in rows if isinstance(row, dict)]
        return results
...
def select_best_openfigi_mapping(
    ...
    ordered = sorted(
        ...
        key=lambda row: (
            ...
            1 if str(row.security_type or "").lower() in {"common stock", "depositary receipt", "equity warrant", "preferred stock"} else 0,
