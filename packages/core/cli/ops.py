...
def _render_strategy_evidence_ledger(console: Any, payload: dict[str, Any]) -> None:
    ...
    for row in rows:
        ...
        console.print(
            ...
            f"raw/post/ret {_render_value(candidates.get('raw_candidate_count'))}/"
            f"{_render_value(candidates.get('postprocess_candidate_count'))}/"
            f"{_render_value(candidates.get('returned_candidate_count'))} "
            f"{_render_value(candidates.get('candidate_productivity_state'))}; "
            f"feat {_render_value(candidates.get('feature_snapshot_count'))} "
            f"{_render_count_map(candidates.get('market_data_quality_state_counts'), limit=2, length=28)}; "
