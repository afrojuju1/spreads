...
- Preserve the separation between source, candidate quality, strategy selection, allocation, portfolio admission, execution admission, execution lifecycle, and position management.
...
| Per-strategy selection | `EntrySelectionEngine` performs account-agnostic quality analysis and selects per-strategy promotable ideas. | Partial | LEAN alpha/signal separation | Current proof is limited when no strategy produces natural candidates/signals, but selected decisions now feed portfolio allocation instead of direct admission. | `spr-9v2.1`, then `spr-h2b` |
| Portfolio admission | `portfolio_admission` blocks duplicate symbol/family exposure, caps, daily entries, correlated groups, max risk, and unallocated decisions. | Shipped | LEAN portfolio construction plus Nautilus risk denials | Admission now consumes AllocationPlan evidence; richer optimization and queued reallocation can still evolve later. | `spr-h2b` |
| Portfolio construction | `AllocationPlan` ranks observed same-day selected decisions across active strategies before portfolio admission, under capital, buying power, strategy budgets, duplicate/correlation exposure, active intents/attempts, open positions, and schedule evidence. | Partial | LEAN portfolio construction | First pass is stored-facts/observed-decision allocation, not a full optimizer or deferred batch auction. | `spr-h2b` |
...
| Historical evaluation | Backend primitive `services/strategy_lab/historical_evaluator.py` ships `build_historical_strategy_evaluation` for bounded `stored_facts_current_model` windows over current ticker-source, candidate, signal, decision, admission, intent, attempt, position, and ClickHouse market-data facts. | Partial | LEAN backtests, Qlib recorder, vectorbt compact research | The first mode compares current catalog strategy/profile/source variants by persisted facts and config hash with explicit fidelity labels; true profile/source reruns and persisted experiment artifacts remain future work. No old replay/audit/backtest/analyze CLI was revived. | `spr-9v2.4`, then feature-store/profile-rerun child |
...
| Qlib | Experiment recorder and research workflow | Offline company valuation has research lanes; trading has engine facts and first stored-fact historical evaluator | Persisted experiment recorder plus true profile/source rerun variants | ML-first live decision engine |
| vectorbt | Compact signal/position research semantics | `build_historical_strategy_evaluation` gives compact current-model decision/outcome comparison over stored facts | Faster comparison layer once feature snapshots and outcomes are persisted more densely | Treating vectorized research as live runtime truth |

## Roadmap Matrix

| Order | Workstream | Start condition | Target capability | Validation |
| --- | --- | --- | --- | --- |
| 0 | Restore candidate production feedback loop | Active strategies are healthy but producing no candidates/signals/selected decisions | Quantified attribution for source-universe gaps, no-raw-candidate builder gaps, raw chain/data coverage gaps, data-quality rejects, calendar-policy rejects, ranking/profile threshold rejects, selection/admission rejects, and true market-condition rejections; fix only proven blockers without fake rows or forced loosening | `spr-9v2.1`, `spr-9v2.3`; strategy ledger shows stage-attributed candidates/signals or quantified no-candidate evidence; config validate, ops state, storage state, jobs, and strategy ledger remain healthy |
| 1 | Historical strategy lab/backend evaluator | Current live blocker attribution identifies what must be measured over history | First backend evaluation surface over current ticker-source, candidate, signal, decision, admission, intent, attempt, position, and ClickHouse market-data facts is shipped as `stored_facts_current_model`; future children can add profile/source rerun mode and persisted experiment artifacts | Config validate, narrow evaluator smoke over stored facts, ClickHouse coverage/fidelity report, artifact output with no old wrapper resurrection |
...
| 3 | Portfolio construction/allocation | Active multi-strategy facts are reliable enough to compare candidates | First-pass cross-strategy `AllocationPlan` ranks selected signals under capital, risk, correlation, strategy caps, and schedule evidence before admission | Portfolio admission consumes allocation output; ops exposes latest allocation decisions; future children can add richer queued optimization |
...
| 6 | Stale runtime noun cleanup | New runtime paths no longer depend on legacy fields | Keep active runtime/config surfaces free of duplicate posture flags; keep `execution.mode` authoritative | `uv run spreads config validate --json` shows no misleading legacy posture fields; docs and skills remain aligned |
