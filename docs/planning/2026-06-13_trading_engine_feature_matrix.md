...
| Backtest evaluation | Backend primitive `services/backtest/` ships `BacktestEngine` with a bounded `stored_facts` mode over current ticker-source, candidate, signal, decision, admission, intent, attempt, position, and ClickHouse market-data facts. | Partial | LEAN backtests, Qlib recorder, vectorbt compact research | The first mode compares current catalog strategy/profile/source variants by persisted facts and config hash with explicit fidelity labels; true profile/source reruns and persisted experiment artifacts remain future work. No old replay/audit/analyze CLI or removed historical backtest command was revived. | `spr-u44`, then child beads |
...
| vectorbt | Compact signal/position research semantics | `BacktestEngine` gives compact current-model decision/outcome comparison over stored facts | Faster comparison layer once feature snapshots and outcomes are persisted more densely | Treating vectorized research as live runtime truth |
