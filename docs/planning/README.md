...
- [Single-Authority Execution Lifecycle Storage](./2026-07-15_single_authority_execution_lifecycle_storage.md) for the approved `spr-n65` ADR that keeps the live execution/position tables, removes the abandoned parallel lifecycle schema and duplicate intent-event log, and defines the clean breaking cutover
...
- [ClickHouse Market Data Cutover](./2026-06-12_clickhouse_market_data_cutover.md) for the current raw option tick storage architecture: ClickHouse raw ticks and quote snapshots, Postgres capture summaries, and no Postgres tick partition maintenance
- [Workflow-Lane Activity Dispatch](./2026-07-15_workflow_lane_activity_dispatch.md) for the implemented `spr-g9n` cutover from one monolithic scheduled-job activity dispatcher to lane-bound handler registries behind one shared job-run executor, including the `spr-0zp` single-attempt reliability decision
- [Earnings Event Source Consensus Cutover](./2026-06-15_earnings_event_source_consensus_cutover.md) for the `spr-9v2.2` design to source earnings strategies from a Redis-protected, yfinance-led 30-day event consensus, corroborated by Alpha Vantage, DoltHub, and Finviz, then actionability-filtered through Alpaca.
- [Market Data Storage And Logging Design](./2026-06-05_market_data_storage_and_logging.md) for the superseded `spr-zuy.6` Postgres-partition storage decision kept as historical context
...
- [Market Intel OpenClaw Architecture](./2026-05-01_market_intel_openclaw_architecture.md)
