...
| Backtest evaluation | `services/backtest/` | Backend-only BacktestEngine primitive. The first shipped mode is `stored_facts`: it evaluates bounded date windows over the current ticker-source, candidate, signal, decision, admission, intent, attempt, position, and ClickHouse market-data model. It compares current catalog strategy/profile/source variants from persisted facts and labels source, candidate, decision, execution, PnL, and market-data fidelity explicitly. It is not an operator app UI and does not revive removed replay/audit/analyze wrappers or the removed historical `spreads backtest` command. |
...
| Backtest evaluation | Bounded backtest artifact for current-model historical windows, including candidate productivity, selection quality, admission outcomes, execution/fill assumptions, exit-decision outcomes, position/PnL labels, admission risk context, reason-code attribution, and ClickHouse coverage/fidelity. | `services/backtest/` | Alternate execution orchestration, broker submission, operator UI, or legacy replay/audit/analyze wrappers. |
