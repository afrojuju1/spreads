...
| Shared market context | `services/trading_engine/market_context.py`, `services/trading_engine/market_context_runtime.py` | `MarketContextEngine` computes strategy-independent `MarketContextSnapshot` and `RegimeSnapshot` facts from benchmark `SymbolMarketSlice` inputs or stored benchmark facts. The snapshots describe observed broad-market regime, risk posture, trend strength, volatility state, benchmark evidence, freshness, confidence, data quality, and fidelity labels. They do not choose trades, inspect account state, or submit orders. Persistence, strategy consumption, allocation, ops, and backtest integration remain separate follow-up boundaries. |
...
- `MarketContextEngine` builds `MarketContextSnapshot` and `RegimeSnapshot` facts from shared benchmark market slices or stored benchmark facts. Entry quality, strategies, allocation, ops, and backtests should consume these shared facts as the follow-up beads cut over; the models and engine do not choose trades or evaluate account-aware admission.
