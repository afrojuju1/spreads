...
| Workflow runtime | `packages/config/jobs`, `packages/core/jobs`, `core.workflow_runtime`, `core.workflows` | Declared routines and generated trading-strategy routines are reconciled into first-class routine schedules. Market-session/open/close schedules materialize an exchange-calendar-exact rolling 400-day horizon, including holidays and early closes; the maintenance lane reconciles every six hours to keep that horizon current. Runtime, data, maintenance, valuation, and research workers each bind the stable `run_scheduled_job_activity` provider name to an exact lane-local handler registry. Temporal owns routine identity, overlap, activity retry/backoff, heartbeat timeout, cancellation, recovery, and authoritative execution state. `RoutineActivityRunner` writes rich job outcomes to `job_runs` as the durable domain/operator evidence; `retry_count` projects the Temporal activity attempt and is never an application requeue counter. Completed workflows return a versioned bounded envelope with exact job/run identity, terminal job status, attempt, reason, and a `job_runs` result reference rather than duplicating the persisted result in history. Canonical operator state reads Temporal visibility/description data so healthy pollers cannot mask stuck workflow tasks, Activity retries, retired queues, schedule failures, or terminal-provider/stale-Postgres mismatches. `spreads jobs repair-projection` verifies the exact terminal Temporal run and reuses its existing Postgres result; it refuses active, mismatched, or evidence-missing work and never leases, requeues, or starts workflows. Scheduled and ad-hoc starts share the same workflow/activity path. Lifecycle broker activities and the capture-session workflow remain separate. Lifecycle, runtime, data, maintenance, and capture workflow lanes are required; research and valuation lanes are explicitly optional and disabled by default. The provider adapter is isolated under `core.workflow_runtime`. |
| Temporal platform | `docker-compose.yml`, `ops/temporal`, `docs/runbooks/temporal.md` | The single-node paper cluster runs pinned official `temporalio/server` and UI images. An idempotent one-shot `temporalio/admin-tools` service owns PostgreSQL schema setup before server start; application startup never mutates Temporal schemas. The `default` namespace retains closed histories for seven days. gRPC and UI host ports are loopback-only, Docker health checks the frontend port, daily custom-format backups cover both Temporal stores, and official server plus Python SDK metrics feed the existing NUC Prometheus/Grafana stack. Single-node availability remains an explicit paper-trading constraint. |
...
The daily `postgres_backup` routine writes retained PostgreSQL custom-format artifacts for `spreads`, `temporal`, and `temporal_visibility`. Restore and Temporal server-upgrade procedures live in `docs/runbooks/temporal.md`; do not restore either Temporal store over the live cluster or advance the server without its matching schema-first sequential hop.
...
`quality profile` proves the candidate is structurally and economically worth considering for that family. `selection` chooses the best account-agnostic idea per strategy. `AllocationPlan` ranks the current decision with fresh, unadmitted concurrent selections across active strategies under shared market-context/regime fit, capital, buying power, strategy budgets, duplicate/correlation exposure, active intents/attempts, open positions, and schedule evidence. Prior decisions with immutable admission or intent outcomes are historical evidence, not reservable planned exposure; active intents, attempts, and positions remain enforced through the portfolio exposure authority. Stale or explicitly blocked context expires the selected decision at this boundary. `protection admission` consumes the selected allocation and evaluates account, portfolio, strategy, event/calendar, duplicate exposure, option scenario, and emergency-stop protections before portfolio admission. `portfolio admission` consumes the plan decision and persists the allocation evidence alongside cap checks. `execution admission` validates broker-submission readiness, including leg shape, net debit/credit sign, quote freshness, and adapter support. Pending intents are handed to `execution_lifecycle_start`, which starts deterministic trade/close lifecycle workflows; those workflows prepare or reuse broker attempts and submit them through idempotent broker activities.
...
- Allocation and entry admission in `services/risk/` are the first account-aware boundaries. `risk/allocation.py` ranks the current decision with fresh, unadmitted concurrent selections while excluding prior lifecycle outcomes already owned by admissions, intents, attempts, and positions; `risk/protection.py` applies protection gates, `risk/sizing.py` resolves quantity policy, and `risk/admission.py` emits stable `allocation_plan`, `portfolio_admission`, `capacity_admission`, `execution_readiness`, `reason_codes`, and `blockers` evidence; final execution readiness is explicitly deferred to the execution submit path.
...
- Follow `docs/runbooks/temporal.md` for Temporal backups, sequential server/schema upgrades, rollback, and monitoring validation.
