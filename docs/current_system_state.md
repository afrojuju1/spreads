...
| Execution, exits, and portfolio state | `services/trading_engine/portfolio_runtime.py`, `services/trading_engine/exit_runtime.py`, `services/trading_engine/close_policy.py`, `services/trading_engine/risk_runtime.py`, `services/execution_intents/`, `services/execution/`, `services/session_positions.py`, `services/broker_sync.py`, `services/risk/`, `services/exit_manager.py`, `core.workflows`, `core.engine` | PortfolioEngine owns position projection. ExitEngine owns position-exit snapshots, close-policy evaluation, and durable CloseDecision facts. `risk_runtime.py` owns close admission for position/reconciliation/order readiness before close intent creation. Execution lifecycle start creates deterministic Temporal trade/close workflows from pending intents. Engine events/outbox provide the cross-aggregate lifecycle event spine while broker attempt/order/fill persistence remains in `services/execution/` during the broker-activity cutover. |
...
- `execution_intents` is the control-plane handoff boundary. It carries the resolved executor-profile snapshot and selects an execution runtime before lifecycle workflow start and broker submission.
...
- Company valuation and TradingAgents research are optional offline task queues. Default trading health, live strategy entry/manage, execution admission, and close management must stay healthy without these workers or jobs enabled.
...
| Admission | Account/risk/policy answer to whether an approved idea can be carried now. Entry admission is an `entry_capacity_precheck`; final quote, broker, and submit readiness stays in execution. | `services/risk/admission.py`, `services/execution/`, admission payloads | Account snapshots alone or strategy selection. |
...
The valuation and research task queues are disabled by default in job config and deploy target config. `worker-valuation` and `worker-research` are compose profiles with zero replicas unless intentionally enabled. The live `TradingOpsState` health path should stay focused on trading/data/runtime task queues; optional offline task queues should appear as idle or disabled, not blocked live trading dependencies.
...
Execution modules have explicit owners: `direct_orders.py` owns equity/option attempt construction for direct operator requests and intent-derived option opens, `position_close.py` owns close-by-position attempt creation, `submit.py` owns broker submission service logic used by workflow activities, `sync.py` owns refresh/cancel reconciliation, `attempts.py` owns attempt/order/fill payload sync helpers, `admission.py` owns execution admission payload helpers, and `order_requests.py` owns order payload construction plus live quote quality gates. Natural strategy open intents now start Temporal workflows through `execution_lifecycle_start`; broker workflow activities own attempt preparation, submit, refresh, cancel, and order/fill sync. Before broker submission, the broker activity still runs option-structure guards over canonical legs, family support, executor order style, Alpaca `mleg` usage, net credit/debit sign, max-risk resolvability, quantity, and executor-profile quote freshness; blocked attempts fail with stable execution-admission reason codes. Do not add new callers to the removed package-root dispatch path.
