...
| Workflow runtime | `packages/config/jobs`, `packages/core/jobs`, `core.workflow_runtime`, `core.workflows` | Declared routines and generated trading-strategy routines are reconciled into first-class routine schedules. Lifecycle, runtime, data, maintenance, and capture workflow lanes are required; research and valuation lanes are explicitly optional and disabled by default. The provider adapter is isolated under `core.workflow_runtime`. |
