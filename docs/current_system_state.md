...
| Engine data and scanning | `services/trading_engine/data_runtime.py`, `services/strategy_builders.py`, `services/scanners/` | DataEngine resolves ticker sources/static sources and builds strategy-owned candidate inputs. Scanner, ranking-policy, and runtime candidate filters are delegated build policy and persist diagnostics under engine-owned candidate facts. |
