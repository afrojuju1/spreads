...
`execution_intents` records pending open/manage/close work and lifecycle-start state. The `execution_lifecycle_start` job starts deterministic Temporal trade/close workflows for pending intents, marks claimed intents with workflow metadata, and appends engine events. Broker submit activities are the next cutover; until then, existing execution services still own attempt/order/fill persistence.
...
Execution modules have explicit owners: `direct_orders.py` owns equity/option attempt construction for direct operator requests and intent-derived option opens, `position_close.py` owns close-by-position attempt creation, `submit.py` owns queued broker submission until broker activities replace it, `sync.py` owns refresh/cancel reconciliation, `attempts.py` owns attempt/order/fill payload sync helpers, `admission.py` owns execution admission payload helpers, and `order_requests.py` owns order payload construction plus live quote quality gates. Natural strategy open intents now start Temporal workflows through `execution_lifecycle_start`; the broker-activity cutover will move attempt construction and submit/refresh/cancel ownership into workflow activities and remove `execution_submit`. Before broker submission, the existing queued submit path still runs option-structure guards over canonical legs, family support, executor order style, Alpaca `mleg` usage, net credit/debit sign, max-risk resolvability, quantity, and executor-profile quote freshness; blocked attempts fail with stable execution-admission reason codes. Do not add new callers to the removed package-root dispatch path.
...
- Replace `execution_submit` with broker workflow activities next. It remains only as the temporary broker-submit isolation job while `spr-w3r.2` moves submit/refresh/cancel behind Temporal and deletes the ARQ submit registration.
